...
fn ordered_curves_all_close2(
    ...
    while y0 < y1 {
        let next_y0 = approximate2(y0, y1, order, segs, accuracy, &mut approxes);
        //dbg!(&approxes);
...
fn approximate2(
    ...
    'retry: loop {
        for seg_idx in order {
            ...
            let cubic = slice_bez(cubic, y0, y1);
